...
correct = list(10, 38, 126, 153, 185, 253, 292, 406, 417, 429)

subset <- data[, 2:4] # 11:13 demo - 2:4 obs
...
#demo_nose_x = as.numeric(data[3:1802, 11]) #demo nose x coordinates for all frames
#demo_nose_y = as.numeric(data[3:1802, 12]) #demo nose y coordinates for all frames

#light_x = as.numeric(data[3:1802, 20]) #demo light x coordinates for all frames
#light_y = as.numeric(data[3:1802, 21]) #demo light y coordinates for all frames
...
rec_chosen_embedding = 3#pick lowest point
...
rec_analysis = crqa(ts1 = rescaled_dist_x, 
                    ...
                    r = 0.3, # you can keep playing with this to find something that works
                    normalize = 0, 
