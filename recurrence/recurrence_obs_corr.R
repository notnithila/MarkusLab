...
setwd('/Users/nithilaannadurai/Documents/MATLAB/')

# read in the data (10 frames/sec, each trial is 40 frames)
data = read.csv('./ibns csv/05262023 119 controlDeepCut_resnet50_IBNS Social AERIALJun10shuffle1_255000.csv')

#'./ibns csv/05262023 119 controlDeepCut_resnet50_IBNS Social AERIALJun10shuffle1_255000.csv'
#'./ibns csv/05282023 97 + 119DeepCut_resnet50_IBNS Social AERIALJun10shuffle1_255000.csv'

#correct = list(1146, 1157, 1169, 1180, 1192, 1204, 1216)
correct = list(1301, 1312, 1326, 1340, 1368, 1419, 1434)
...
for (i in 1:length(correct)) {
  frame <- correct[[i]] * 10
  frames_x <- as.numeric(data[frame:(frame + 39), 2])  # Extract the corresponding frames
  frames_y <- as.numeric(data[frame:(frame + 39), 3])
...
for (i in 1:length(correct)) {
  frame <- correct[[i]] * 10
  frames_x <- as.numeric(data[frame:(frame + 39), 11])  # Extract the corresponding frames
  frames_y <- as.numeric(data[frame:(frame + 39), 12])
...
#first 5 mins - level of activity

obs_nose_x_first_5 = as.numeric(data[3:3000, 2]) #obs nose x coordinates for frames (2 mins) - first 74 trials
obs_nose_y_first_5 = as.numeric(data[3:3000, 3]) #obs nose y coordinates for frames 

light_x_first_5 = as.numeric(data[3:3000, 11]) #demo light x coordinates for all frames
light_y_first_5 = as.numeric(data[3:3000, 12]) #demo light y coordinates for all frames

#calculates the squared distance from the obs's nose to the light's x coordinate and stores in dist_x
dist_x_first_5 <- vector("numeric", length = length(obs_nose_x_first_5))
for (i in seq_along(obs_nose_x_first_5)) {
  dist_x_first_5[i] <- (obs_nose_x_first_5[i] - light_x_first_5[i])**2
}

#calculates the squared distance from the obs's nose to the light's y coordinate and stores in dist_y
dist_y_first_5 <- vector("numeric", length = length(obs_nose_y_first_5))
for (i in seq_along(obs_nose_y_first_5)) {
  dist_y_first_5[i] <- (obs_nose_y_first_5[i] - light_y_first_5[i])**2
}

activity_level_x = sum(dist_x_first_5)
activity_level_y = sum(dist_y_first_5)
...
rec_chosen_delay = 15 #first local minimum, finding first iteration
...
rec_analysis = crqa(ts1 = rescaled_dist_x, 
                    ...
                    r = 0.0003, # you can keep playing with this to find something that works
                    normalize = 0, 
...
abline(v=seq(1, 3000, by=40), col="blue")
abline(h=seq(1, 3000, by=40), col="blue")

#adds time stamp labels
axis(side = 1)
axis(side = 2)

legend(1, 281, legend=c("Every trial"),
